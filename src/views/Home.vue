    ...
    <el-container>
      ...
      <scroll :data="openCityList" ref="suggest" :probeType="3" :listenScroll="true" @distance="distance" @isFlagShow="isFlagShow">
        <div>
          ...
          <CityList :hotCityList="hotCityList" :openCityList="openCityList" :elementIndex="elementIndex" @change-city="changeCity" @scrollToLetter="scrollToLetter"/>
        </div>
      </scroll>
    ...
    changeCity (newCity) {
      this.$confirm(`确定切换城市到${newCity.name || newCity.city}?`, {
        ...
        if (newCity !== this.historyCityArr[0]) {
          ...
          this.$router.push({ name: 'about', params: { selectedCity: newCity }});
        }
      }).catch(() => {        
      });
    },
